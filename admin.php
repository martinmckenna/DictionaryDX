...
<?php 
	...
	if(isset($_POST['edit_submit'])){ //checks if a form named submit has been added
		update_word($_POST);
	}
	?>
...
<body>
<div class = "container">
<form method="post" name="delete_word" class = "delete">
<div>
<h1>Select a Word to delete</h1><select multiple name="delete" class = "admin_field" size = 10>
		
			<?php
				$list= listof_words();
				foreach($list as $option){
					print '\t<option value="'.$option['did']. '">'.$option['word'] .'</option>\n';
					}
				?>
	</select>
</div>
...
<div>
	<input type="submit" name="submit" class = "admin_field" value="Submit" />
</div>
</form>


<form method="post" name="edit_word" class = "edit">
<div><h1>Select a Word to Edit</h1>
<select name="edited_word" class = "admin_field">
...
<div>
<p>Updated Word</p>
	<label for="word"></label> <input type="text" class = "admin_field" name="word" placeholder="Enter your word!" maxlength="50"/>
</div>

<div>
<p>Updated Definiton</p>
	<label for="def"></label> <textarea name="def" class = "admin_field" maxlength="1000"> </textarea>
</div>

<div>
<p>Updated Part of Speech</p>
	<select name="pos" class = "admin_field">
...
<div>
	<input type="submit" class = "admin_field" name="edit_submit" value="Submit" />
</div>
</form>
